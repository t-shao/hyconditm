...
@MODEL_REGISTRY.register()
class UnpairedITMModel(ITMGANModel):
    def init_training_settings(self):
        train_opt = self.opt['train']
        ...
        if train_opt.get('gradient_penalty_weight'):
            self.gp_weight = train_opt['gradient_penalty_weight']
            self.cri_gp = gradient_penalty_loss
        else:
            self.cri_gp = None
    ...
    def optimize_parameters(self, current_iter):
        ...
        if (current_iter % self.net_d_iters == 0 and current_iter > self.net_d_init_iters):
            ...
            if self.cri_identity:
                l_g_id = self.cri_identity(self.output, self.lq)
                l_g_total += l_g_id
        ...
        # gradient penalty
        if self.cri_gp:
            l_d_gp = self.gp_weight * self.cri_gp(self.net_d, self.gt, self.output.detach())
...
@MODEL_REGISTRY.register()
class UnpairedContrastiveITMModel(ITMGANModel):
    def init_training_settings(self):
        train_opt = self.opt['train']
        ...
        if train_opt.get('contrastive_opt'):
            ...
            self.nce_layers = train_opt['contrastive_opt']['nce_layers']
        else:
    ...
    def optimize_parameters(self, current_iter):
        ...
        if (current_iter % self.net_d_iters == 0 and current_iter > self.net_d_init_iters):
            ...
            if self.cri_contrastive:
                l_g_contrastive = self.calculate_NCE_loss(self.output, self.lq)
                l_g_total += l_g_contrastive
                loss_dict['l_g_contrastive'] = l_g_contrastive
            # pixel loss
            if self.cri_pix:
                l_g_pix = self.cri_pix(self.output, self.gt)
                l_g_total += l_g_pix
    ...
    def calculate_NCE_loss(self, src, tgt):
        ...
        feat_q = self.net_g(tgt, self.nce_layers, encode_only=True)

        if self.opt.flip_equivariance and self.flipped_for_equivariance:
            feat_q = [torch.flip(fq, [3]) for fq in feat_q]

        feat_k = self.net_g(src, self.nce_layers, encode_only=True)
        feat_k_pool, sample_ids = self.netF(feat_k, self.opt.num_patches, None)
        feat_q_pool, _ = self.netF(feat_q, self.opt.num_patches, sample_ids)

        total_nce_loss = 0.0
        for f_q, f_k, crit in zip(feat_q_pool, feat_k_pool, self.cri_contrastive):
            loss = crit(f_q, f_k)
...
class MidSupITMModel(ITMModel):
    ...
    def optimize_parameters(self, current_iter):
        ...
        if self.cri_perceptual_mid:
            l_percep_mid, l_style_mid = self.cri_perceptual_mid(self.output_mid, self.gt)
            if l_percep_mid is not None:
                l_total += l_percep_mid
                loss_dict['l_percep_mid'] = l_percep_mid
            if l_style_mid is not None:
                l_total += l_style_mid
                loss_dict['l_style_mid'] = l_style_mid
        if self.cri_color_mid:
            l_color_mid = self.cri_color_mid(self.output_mid, self.gt)
            l_total += l_color_mid
            loss_dict['l_color_mid'] = l_color_mid
