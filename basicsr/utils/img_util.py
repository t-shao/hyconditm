...
def imread(path, float32=False, bit_depth=None):
    """
    read image file.

    :param path: image file path.
    :param bgr2rgb: whether to convert the channel order
        from BGR to RGB.
    :param bit_depth: bit depth of the image.
    :return: np.ndarray, np.float32, HWC, [0, 1].
    """
...
def imwrite(img, file_path, params=None, auto_mkdir=True, bit_depth=None, rgb2bgr=False):
    """Write image to file.
    ...
    if bit_depth == 8:
        ...
        img = (img * 65535.).round().astype(np.uint16)
